...
def compute_segmentation_scores(prediction_mask, reference_mask, voxel_spacing):
    ...
    for i, obj_id in enumerate(np.unique(prediction_mask)):
        ...
        # Limit processing to the bounding box containing both the prediction and reference objects.
...
def detect_lesions(prediction_mask, reference_mask, min_overlap=0.5):
    ...
    # Remove the false positive that have none intersection with ref
    ...
    temp = ndimage.find_objects(target_mask)
    bounding_box = ndimage.find_objects(target_mask)[0]
    ...
    # Merge and label reference lesions that are connected by predicted lesions.
    ...
    for i, p_id in enumerate(p_id_list):
        ...
        # label order. Only merge those g_id which overlap this p_id more than others.
        g_id_merge = []
        g_id_merge_indices = []
    ...
    # Merge and label predicted lesions that are connected by reference lesions.
...
def test():
    ...
    # print(num_detected)
    voxel_spacing = [1,1]
    compute_segmentation_scores(pred_mask_lesion, ref_mask_lesion, voxel_spacing)
